...
def load_documents(source_dir: str, ignored_files: List[str] = []) -> List[Document]:
    ...
    with Pool(processes=os.cpu_count()) as pool:
        ...
        with tqdm(
            ...
            ):  # Call loan_single_document for each file path in parallel
                results.extend(docs)
                pbar.update()
...
def process_documents(ignored_files: List[str] = []) -> List[Document]:
    ...
    )  # split the docs into text chunks
...
def main():
    ...
    print(
        f"Ingestion complete! You can now run app.py to start querying your documents."
    )
